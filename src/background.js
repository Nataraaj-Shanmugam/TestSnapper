// Enhanced Background service worker for TestSnapper with multi-format export
class TestSnapperBackground {
  constructor() {
    ...
    this.screenshotQueue = []; // Queue for screenshots during recording
    this.init();
  }
  ...
  async captureScreenshot(context = {}) {
    ...
    try {
      ...
      if (this.currentSession) {
        ...
        console.log('Screenshot captured:', context.type || 'manual');
      }
  ...
  async recordInteraction(interaction, tab) {
    ...
    // Auto-capture screenshot for important interactions
    const importantInteractions = ['click', 'input', 'navigation', 'url_change'];
    if (importantInteractions.includes(interaction.type)) {
      setTimeout(() => {
        this.captureScreenshot({ 
          type: 'interaction', 
          interactionType: interaction.type,
          interactionId: recordedInteraction.id
        });
      }, 100); // Small delay to capture post-interaction state
    }
  ...
  exportAsTxt(session) {
    ...
    txtContent += `Screenshots: ${session.screenshots?.length || 0}\n\n`;
    ...
    if (session.screenshots && session.screenshots.length > 0) {
      ...
      session.screenshots.forEach((screenshot, index) => {
        ...
        if (screenshot.context.interactionType) {
          txtContent += `   Related to: ${screenshot.context.interactionType}\n`;
        }
      });
    }
  ...
  exportAsDocx(session) {
    // This would require a DOCX library like docx or similar
    // For now, return structured data that could be processed by a DOCX library
    const docxData = {
      ...
      sections: [
        {
          ...
          content: `This session was recorded on ${new Date(session.startTime).toLocaleString()} at ${session.url}. The total duration was ${Math.round((session.duration || 0) / 1000)} seconds with ${session.interactions.length} interactions captured.`
        },
        {
          title: 'Interactions',
  ...
  exportAsPdf(session) {
    // This would require a PDF library like jsPDF or similar
    // For now, return structured data that could be processed by a PDF library
    const pdfData = {
      title: `TestSnapper Session: ${session.name}`,
      metadata: {
        url: session.url,
